...
class User extends Component {
    ...
    componentDidMount() {
        ...
        const birds = firebase.firestore().collection('captures').where("userUid", "==", this.props.match.params.id);
        birds.get().then((doc) => {
            this.setState({
            ...
            return (
                <React.Fragment>
                    <section>
                        ...
                        <h3 className="section_subtitle">Tout les captures de {user.name}</h3>
                        <ul className="list">
                            {birds.map(bird => (
                                <li key={bird.id} className="list__item">
                                    ...
                                    <Link to={{pathname: '/captures/' + bird.id}} className="icon"><span className="sro">Modifier</span>
